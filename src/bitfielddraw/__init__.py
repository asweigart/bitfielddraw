...
# TODO - center parameter still has bugs. Try it with: s('(x^y)%5', w=1000,h=1000,center=True)
...
def _adjustParams(func, left, bottom, width, height, center, **kwargs):
    """
...
def getBitField(func, invert=False, left=0, bottom=0, width=None, height=None, flipx=False, flipy=False, center=False, **kwargs):
    """
    ...
    """
    func, left, bottom, width, height = _adjustParams(func, left, bottom, width, height, center, **kwargs)
...
def getBitFieldImg(func, invert=False, left=0, bottom=0, width=None, height=None, flipx=False, flipy=False, center=False, fg='white', bg='black', **kwargs):
    """
    ...
    draw = ImageDraw.Draw(im)
    pixels = []
    for xy in getBitField(func, invert=invert, left=left, bottom=bottom, width=width, height=height, flipx=flipx, flipy=flipy, center=False, **kwargs):
...
def saveBitFieldImg(filename, func, invert=False, left=0, bottom=0, width=None, height=None, flipx=False, flipy=False, center=False, fg='white', bg='black', **kwargs):
    """
    ...
    :param bg: The Pillow color string for background pixels where func returns a zero value. Can be a word like 'red' or RGB value like '#ff0000'. Defaults to 'black'.
    """
    getBitFieldImg(func, invert=invert, left=left, bottom=bottom, width=width, height=height, flipx=flipx, flipy=flipy, center=center, fg=fg, bg=bg, **kwargs).save(filename)


def showBitFieldImg(func, invert=False, left=0, bottom=0, width=None, height=None, flipx=False, flipy=False, center=False, fg='white', bg='black', **kwargs):
    """
    Saves an image of the bit field produced by func.
    ...
    :param bg: The Pillow color string for background pixels where func returns a zero value. Can be a word like 'red' or RGB value like '#ff0000'. Defaults to 'black'.
    """
    getBitFieldImg(func, invert=invert, left=left, bottom=bottom, width=width, height=height, flipx=flipx, flipy=flipy, center=center, fg=fg, bg=bg, **kwargs).show()


def getBitFieldStr(func, invert=False, left=0, bottom=0, width=None, height=None, flipx=False, flipy=False, center=False, **kwargs):
    """
...
def randFunc(numOps=5):
    if numOps <= 0:
        return ''

    operand, numOps = _randomOperand(numOps)
    func = operand
...
def _randomOperand(numOps):
    func = ''
    if random.randint(1, 3) == 1:
        # Put another func as the operand:
        numOps -=1
        thingie = randFunc(numOps - 1)
        if thingie != '':
            func += '(' + thingie + ')'
            return func, numOps

    # Use x, y, or a number as the operand:
    if random.randint(1, 3) == 1:
        # Use x or y
        if 'x' in func and 'y' not in func:
            func += 'y'  # Use y since it hasn't been used yet.
        elif 'x' not in func and 'y' in func:
            func += 'x'  # Use x since it hasn't been used yet.
        else:
            # Randomly decide between x and y:
            if random.randint(1, 2) == 1:
                func += 'x'
            else:
                func += 'y'
    else:
        func += str(random.randint(1, 20))
    return func, numOps
